        ...
        <?php foreach ((array) $layout['menu_items'] as $key => $menu_item): ?>
            <section class="profile-body listing-tab tab-hidden <?php echo esc_attr( "tab-type-{$menu_item['page']}" ) ?> <?php echo esc_attr( sprintf( 'tab-layout-%s', ! empty( $menu_item['template'] ) ? $menu_item['template'] : 'masonry' ) ) ?> pre-init" id="listing_tab_<?php echo esc_attr( $menu_item['slug'] ) ?>">
                ...
                <?php if ($menu_item['page'] == 'main' || $menu_item['page'] == 'custom'):
                    ...
                    // And divide them with an 'endcolumn' array item, which will later be used to construct columns.
                    ...
                    <div class="container <?php printf( 'tab-template-%s', $menu_item['template'] ) ?>">
                        <div class="row <?php echo $menu_item['template'] == 'masonry' ? 'listing-tab-grid' : '' ?>">
                            ...
                            <?php $id = get_the_ID();
                            $prodId = get_post_meta($id, '_product-id');
                            $product = wc_get_product( $prodId[0] );

                            $post_data = json_encode( [
                                'id'	=> $listing->get_id(),
                                'image'	=> get_avatar_url( $listing->get_author_id(), 32 ),
                                'title'	=> $listing->get_author()->first_name. " " .$listing->get_author()->last_name. " for the ".$listing->get_name(),
                            ...
                            <div class="col-md-12 block-type-categories">
                                <div class="element content-block plain-text-content" id="bookingInfo">
                                    ...
                                    <div class="pf-body">
                                        ...
                                        <?php if (strlen($PricePercentage) > 0) {
                                            echo "<p>The rest of the payment should be paid ".strtolower($PricePercentage)."</p>";
                                        } ?>
                                    </div>
                                </div>
                            </div>
