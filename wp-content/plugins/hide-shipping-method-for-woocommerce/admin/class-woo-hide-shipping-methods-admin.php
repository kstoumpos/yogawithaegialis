...
class Woo_Hide_Shipping_Methods_Admin
{
    ...
    public function whsma_enqueue_styles( $hook )
    {
        ...
        if ( false !== strpos( $hook, 'woocommerce_page_whsm' ) ) {
            wp_enqueue_style(
    ...
    public function whsma_enqueue_scripts( $hook )
    {
        ...
        if ( false !== strpos( $hook, 'woocommerce_page_whsm' ) ) {
            ...
            wp_localize_script( $this->plugin_name, 'coditional_vars', array(
                ...
                'validation_length1'             => esc_html__( 'Please enter 3 or more characters', 'woo-hide-shipping-methods' ),
                'product_placeholder'            => esc_html__( 'Please enter 3 or more characters', 'woo-hide-shipping-methods' ),
                'category_placeholder'           => esc_html__( 'Select category', 'woo-hide-shipping-methods' ),
                'select_some_options'            => esc_html__( 'Select some options', 'woo-hide-shipping-methods' ),
                'select_category'                => esc_html__( 'Select Category', 'woo-hide-shipping-methods' ),
                'delete'                         => esc_html__( 'Delete', 'woo-hide-shipping-methods' ),
                'validation_length2'             => esc_html__( 'Please enter', 'woo-hide-shipping-methods' ),
                'validation_length3'             => esc_html__( 'or more characters', 'woo-hide-shipping-methods' ),
                ...
                'click_here'                     => esc_html__( 'Click Here', 'woo-hide-shipping-methods' ),
                'current_url'                    => $current_url,
                'doc_url'                        => "https://www.thedotstore.com/docs/plugin/hide-shipping-methods-for-woocommerce/",
    ...
    public function whsma_product_fees_conditions_values_ajax()
    {
        ...
        if ( 'country' === $condition ) {
            ...
            $html .= wp_json_encode( $this->whsma_get_product_list( $count, [], true ) );
     ...
     *
     */
    public function whsma_get_product_list( $count = '', $selected = array(), $json = false )
    {
     ...
     *
     */
    public function whsma_get_category_options( $selected = array(), $json )
    {
        global  $sitepress ;
        $default_lang = $this->whsma_get_default_language_with_sitepress();
     ...
     *
     */
    public static function whsma_get_shipping_method()
    {
        $sm_args = array(
    ...
    public function whsma_updated_message( $message, $tab, $validation_msg )
    {
        ...
        if ( 'woo_hide_shipping' === $tab ) {
            ...
            if ( 'saved' === $message ) {
                $updated_message = esc_html__( "Settings saved successfully", 'woo-hide-shipping-methods' );
            }
    ...
    {
        $whsm_default_cslist = array( 'flat_rate', 'free_shipping', 'local_pickup' );
     ...
     * @since 1.0.0
     */
    public function whsma_pro_admin_footer_review()
    {
        $url = '';
        $url = esc_url( 'https://wordpress.org/plugins/woo-hide-shipping-methods/#reviews' );
        $html = sprintf(
