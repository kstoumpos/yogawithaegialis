...
if(!empty($perfmatters_ga['enable_local_ga']) && $perfmatters_ga['enable_local_ga'] == "1") {
	...
	if(empty($perfmatters_ga['script_type']) || $perfmatters_ga['script_type'] == 'gtag' || $perfmatters_ga['script_type'] == 'gtagv4') {
		...
		if($perfmatters_ga['script_type'] == 'gtag') {
...
function perfmatters_update_ga() {
	...
	if(!empty($perfmatters_ga['script_type']) && ($perfmatters_ga['script_type'] == 'gtag' || $perfmatters_ga['script_type'] == 'gtagv4')) {
		if(!empty($perfmatters_ga['tracking_id'])) {
			$queue[$perfmatters_ga['script_type']]= array(
				'remote' => 'https://www.googletagmanager.com/gtag/js?id=' . $perfmatters_ga['tracking_id'],
				'local' => WP_CONTENT_DIR . '/uploads/perfmatters/' . $perfmatters_ga['script_type'] . '.js'
			);
		}
	}
		...
		foreach($queue as $type => $files) {
			if(!empty($files['remote']) && !empty($files['local'])) {
			    ...
			    if($type == 'gtag') {
			    	...
			    	$uploads_url = (!empty($perfmatters_ga['cdn_url']) ? trailingslashit($perfmatters_ga['cdn_url']) . 'wp-content' : content_url()) . '/uploads/perfmatters';
...
function perfmatters_print_ga() {
	...
	elseif($perfmatters_ga['script_type'] == 'gtag') {
		$output.= '<script src="' . content_url() . '/uploads/perfmatters/gtag.js?id=' . $perfmatters_ga['tracking_id'] . '"></script>'; 
	...
	elseif($perfmatters_ga['script_type'] == 'gtagv4') {
    	$output.= '<script src="' . content_url() . '/uploads/perfmatters/gtagv4.js?id=' . $perfmatters_ga['tracking_id'] . '"></script>'; 
...
function perfmatters_monster_ga_gtag($url) {
	return content_url() . '/uploads/perfmatters/gtag.js';
}
