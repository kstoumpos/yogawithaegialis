...
function perfmatters_lazy_load_init() {
	...
	if(!empty($perfmatters_options['lazy_loading']) || !empty($perfmatters_options['lazy_loading_iframes'])) {
		...
		if(!$exclude_lazy_loading && !is_admin() && !wp_doing_ajax() && !isset($_GET['perfmatters']) && !perfmatters_is_page_builder() && !is_embed() && !is_feed()) {
			...
			add_filter('rocket_exclude_defer_js', 'perfmatters_lazy_load_exclude_wp_rocket');
...
//exclude our lazy load script from wp rocket deferral
function perfmatters_lazy_load_exclude_wp_rocket($exclude_defer_js) {
	$exclude_defer_js[] = 'lazyload.min.js';
